...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		BTreeInternalPage BTIPage = (BTreeInternalPage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
		Iterator<BTreeEntry> entryItr = BTIPage.iterator();
		...
		if(entry == null) {
			throw new DbException("no entries found.");
		}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		for(int i = 0; i < tupleNum / 2; ++i) {
			Tuple tuple = tupleItr.next();
			page.deleteTuple(tuple);
			rightPage.insertTuple(tuple);
		}

		//update the pointer, if there is right page exist
		...
		BTreeEntry entry = new BTreeEntry(rightPage.iterator().next().getField(keyField), page.getId(), rightPage.getId());
		...
		if(page.getRightSiblingId() != null) {
			...
			rightPage.setRightSiblingId(rightSibling.getId());
		}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> entryItr = page.reverseIterator();

		if(entryItr == null || !entryItr.hasNext())
		...
		page.deleteKeyAndRightChild(entry);
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), entry.getKey());
		parent.insertEntry(new BTreeEntry(entry.getKey(), page.getId(), rightPage.getId()));
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int numTuples = sibling.getNumTuples() + page.getNumTuples();
		int moveTuplesCount = numTuples / 2 - page.getNumTuples();
		...
		if(isRightSibling)
			tupleItr = sibling.iterator();
		else
			tupleItr = sibling.reverseIterator();
		if(tupleItr==null || !tupleItr.hasNext())
			throw new DbException("sibling has no tuple");
		
		// move tuples from the sibling to the page
		for(int i =0;i < moveTuplesCount;i++) {
			tuple = tupleItr.next();
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> entryItr = leftSibling.reverseIterator();
		
	for(int i = 0; i < moveEntriesCount-1;i++) {
			BTreeEntry e = entryItr.next();
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int totalTuple = rightPage.getNumTuples();
		for(int i =0; i<totalTuple; i++) {
			Tuple tup = tupleItr.next();
		...
		if(rightId != null) {
			BTreeLeafPage rightSibling = (BTreeLeafPage) getPage(tid, dirtypages, rightId, Permissions.READ_WRITE);
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> entryItr = rightPage.iterator();
		if(entryItr.hasNext()) {
			BTreeEntry firstEntry = entryItr.next();
