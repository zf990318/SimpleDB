...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        // some code goes here
        int smaller = Math.min(card1, card2);
        int larger = Math.max(card1, card2);
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        return planCache.getOrder(LJNS);
    }
    ...
    private CostCard computeCostAndCardOfSubplan(
        ...
        } else if (doesJoin(prevBest, table1Alias)) {
            cost = estimateJoinCost(j, t1card, t2card, t1cost, t2cost);
        } else if (doesJoin(prevBest, table2Alias)) {
            LogicalJoinNode j2 = j.swapInnerOuter();
